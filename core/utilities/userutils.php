<?php
    ...
    string $accesskey = ''
		...
		public static function LoginUser(string $username, string $password): string|array {
			...
			if(sizeof($errors) != 0) {
				error_log("LoginUser validation errors: " . print_r($errors, true));
				return $errors;
			}
			...
			if($result_grabuser->num_rows == 1) {
				...
				if(password_verify($pass_password, $user_row['user_password'])) {
					error_log("LoginUser: Password verified successfully for $username");
					...
					error_log("LoginUser: Password verification failed for $username");
				}
			} else {
				error_log("LoginUser: User not found: $username");
			}
		...
		static function SetCookies(string $security): void {
			...
			setcookie("ANORRLSECURITY", $security, time() + (460800* 30), "/");
		}
