<?php
	...
	class User {
		...
		function getRecentlyPlayedGames(int $limit = 2): array {
			// Use GROUP BY with ORDER BY MAX(time) to be compatible with strict SQL modes
			$rows = Database::singleton()->run(
				"SELECT `place` FROM `visits` WHERE `player` = :id GROUP BY `place` ORDER BY MAX(`time`) DESC LIMIT :limit", 
				[
					":id" => $this->id,
					":limit" => $limit
				]
