	...
	class Status {
		...
		public static function Send(int $userid, string $contents) {
			...
			if($user != null && !$user->IsBanned()) {
				...
				if($latest_status != null && !$user->HasProfileBadgeOf(ANORRLBadge::ADMINISTRATOR)) {
					// check if user hasn't posted one in 15s
				...
				if(strlen($status_content) < 4) {
					return ["error"=> true, "reason" => "Status is too short change it. (4 characters minimum)"];
				}
				if(strlen($status_content) > 64) {
					return ["error"=> true, "reason" => "Status is too long change it. (64 characters maximum)"];
				}
